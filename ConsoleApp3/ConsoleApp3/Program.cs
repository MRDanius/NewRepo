...
namespace philosophers_os
{
    ...
    class Flunky
    {
        ...
        public Flunky(int maxPhilosophers)
        {
            semaphore = new Semaphore(maxPhilosophers - 1, maxPhilosophers - 1);
        }
    ...
    class Philosopher
    {
        ...
        public Philosopher(int number, Fork left, Fork right, Flunky flunky, bool dbg)
        {
            this.id = number;
            this.fork_left = left;
    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            Flunky flunky = new Flunky(N);

            Philosopher[] phils = new Philosopher[N];
            for (int i = 0; i < N; i++)
            {
                phils[i] = new Philosopher(i + 1, forks[(i + 1) % N], forks[i], flunky, dbg);
            }
